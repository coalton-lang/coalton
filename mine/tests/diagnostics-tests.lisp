...
(defun check-quick-result-selection-range ()
  (let ((cs (cursor:cursor-new))
    ...
    (cursor:cursor-move-to-position! cs 3)
    (cursor:cursor-start-selection! cs)
    (%check (eq (mine/app/mine::%active-selection-range gb cs) coalton:none)
  ...
  (let* ((text (concatenate 'string "a" (string #\Return) (string #\Newline) "b"))
    ...
    (let ((range (mine/app/mine::%active-selection-range gb cs)))
      (%check (and (not (eq range coalton:none))
                   (= (%tuple-slot range "_0") 1)
                   (= (%tuple-slot range "_1") 3))
              "Expected selection touching LF in CRLF to expand to 1..3, got ~S"
              range))))
