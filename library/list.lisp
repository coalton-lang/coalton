  ...
  (define (equivalence-classes-by f l)
    ...
    (rec g ((remaining l)
      ...
      (match remaining
        ((Nil) partitions)
        ((Cons x rst)
         (match (partition (fn (y) (f x y)) rst)
           ((Tuple yes no)
            (g no (Cons (Cons x yes) partitions))))))))

  (declare equivalence-classes (Eq :a => ((List :a) -> (List (List :a)))))
  (define (equivalence-classes l)
    (equivalence-classes-by == l))
