  ...
  (define-instance (Unfoldable LispArray)
    (define (unfold proc seed)
      (let ((buf (lisp :a ()
        ...
        (rec next ((seed seed))
          (match (proc seed)
            ((None) (lisp (LispArray :b) (buf)
                      (cl:coerce buf 'cl:simple-vector)))
            ((Some (Tuple seed x))
             (lisp Unit (buf x)
               (cl:vector-push-extend x buf)
               Unit)
             (next seed))))))
    (define (unfoldr proc seed)
      (let ((buf (lisp :a ()
        ...
        (rec next ((seed seed))
          (match (proc seed)
            ((None) (lisp (LispArray :b) (buf)
                      (cl:let* ((len (cl:fill-pointer buf))
                                (res (cl:make-array len)))
                        (cl:loop :for i :below len
                           :do (cl:setf (cl:aref res (cl:- len i 1))
                                        (cl:aref buf i)))
                        res)))
            ((Some (Tuple x seed))
             (lisp Unit (buf x)
