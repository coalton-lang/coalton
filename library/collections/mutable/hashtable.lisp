(coalton-library/utils:defstdlib-package #:coalton-library/collections/mutable/hashtable
  (:use
   #:coalton
...
(in-package #:coalton-library/collections/mutable/hashtable)
...
; #+sb-package-locks
; (sb-ext:lock-package "COALTON-LIBRARY/COLLECTIONS/MUTABLE/HASHTABLE")
...
(coalton-toplevel
  ...
  (define (remove-duplicates! iter)
    ...
    (let ((already-seen (coalton-library/collections/mutable/hashtable:new))
          (unique? (fn (elt)
                     (match (coalton-library/collections/mutable/hashtable:get already-seen elt)
                       ((Some _) False)
                       ((None)
                        (coalton-library/collections/mutable/hashtable:set! already-seen elt Unit)
                        True)))))
      (filter! unique? iter))))
