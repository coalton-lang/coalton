...
(coalton-toplevel
  ...
  (declare reverse-bits (UFix -> UFix -> UFix))
  (define (reverse-bits n x)
    "Reverse the first `n` bits of `x` and set the rest to 0."
    #+(and SBCL ARM64)
    (rbit:rbitn n x)
    #-(and SBCL ARM64)
    (rec (% (UFix -> UFix -> UFix -> UFix))
        ((x x) (res 0) (remaining-bits n))
      (if (positive? remaining-bits)
          (% (bits:shift -1 x)
             (bits:or (bits:shift 1 res) (bits:and x 1))
             (1- remaining-bits))
          res))))
