(defpackage #:coalton-benchmarking/benchmarking
  ...
  (:export
   ...
   #:define-parameterized-benchmark
...
(coalton-toplevel
  ...
  (define-struct Benchmark
    ...
    (code
     ...
     (Unit -> Unit))
    (Detect-convergence?
     "Should this benchmark detect convergence?

This enambles standard deviation information and dynamic sample scaling, which treats the number of samples as a minimum number, then checks for a convergence of standard deviation before finishing the run.

This mode sacrifices time efficiency."
     Boolean))
...
Key is package name."
  ...
  (define (%define-benchmark name samples fn detect-convergence?)
...
(cl:defmacro define-benchmark (name samples func cl:&key (detect-convergence? cl:nil))
  "Define a Coalton `Benchmark` in the local package- called outside of Coalton."
  (cl:let* ((name (cl:string name)))
    `(coalton (%define-benchmark ,name ,samples ,func (lisp Boolean () ,detect-convergence?)))))
...
(coalton-toplevel
  ...
  (define-struct (ParameterizedBenchmark :a)
    "Parameterized benchmarks allow for multiple inputs."
    (name
...
If comprehensive, this is the minimum number of samples."
  ...
  (define (%add-parameterized-benchmark (ParameterizedBenchmark name samples func inputs detect-convergence?))
    "Adds a benchmark for each ParameterizedBenchmark input."
...
(cl:defmacro define-parameterized-benchmark (name samples func inputs cl:&key (detect-convergence? cl:nil))
  "Defines a series of benchmarks on a function taking a series of inputs."
  (cl:let ((name (cl:string name)))
    `(coalton (%add-parameterized-benchmark
               (ParameterizedBenchmark
                ,name
                ,samples
                ,func
                ,inputs
                (lisp Boolean () ,detect-convergence?))))))
;;;
;;; Benchmark Results
;;;
...
(coalton-toplevel
  ...
  (define-struct BenchmarkResults
    ...
    (samples
     ...
     UFix)
    (total-time-elapsed
     "The amount of time in internal time units that all samples took to run."
     Integer)
    (average-time-elapsed
     "The amount of time in internal time units that the average sample took to run."
     Integer)
    (time-std-dev
...
(coalton-toplevel
  ...
  (define (column-values (BenchmarkResults name samples time-elapsed avg-time time-std-dev bytes-consed))
    ...
    (seq:make (TableCell (the String (into name)) Left)
              ...
              (TableCell (unwrap-or-else (fn (x)
                                         ...
                                         bytes-consed)
                         Right)
              (TableCell (the String (into samples))
                         Right)))
...
(coalton-toplevel
  ...
  (define (%run-benchmark (Benchmark name samples func _))
    "Runs a `Benchmark`."
    (let profile = (sys:spacetime (fn ()
                                    (for i in (iter:up-to samples)
                                      (func)
                                      Unit))))
    (BenchmarkResults
     name
  ...
  (define (run-package-benchmarks name)
    ...
    (for b in (package-benchmarks name)
      (let res = (if (.detect-convergence? b)
                     (%run-comprehensive-benchmark b)
                     (%run-benchmark b)))
      (when (verbose?)
