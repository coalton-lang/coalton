...
(coalton-toplevel
  ...
  (define (%column-spacing width columns)
    "Evenly divides the width by the number of columns. Returns the size for each column plus a leftovers for the last column."
    (if (== columns 1)
        (Tuple width 0)
  ...
  (define (%bottom-edge width columns)
    "Generates the top-edge of a row of width `width` divided evenly into `columns` columns"
    (%write-component width columns BottomRight BottomUp BottomRight))
