...
Coaltonは命令的なループ、条件付きループ、およびパターンに基づく反復処理をサポートしています。
...
以下は、`:while` 節を使って条件が真である間だけループします。
...
以下は、`for` と `match` と `break` を組み合わせて、パターンがマッチする限り処理を続けます。
...
`for` は、`break` と `continue` をサポートしています。

`break` は、反復処理を即座に終了します。以下のコードは、`0`、`1`、`2` を表示して終了します。
...
以下の例では、偶数値をスキップして `1`、`3`、`5` を出力します。
...
`for` は、任意のループラベルをとります。これらのラベルは、`break`と`continue`と組み合わせて使用することで、複雑な制御フローを実現できます。
...
以下の完全に実用的ではない例では、最も外側のループは`:outer`というラベルでマークされています。このラベルは、内側の`for`内から`break`で参照され、 `x` と `y` の合計が500を超えるたびに反復を終了させます。`:outer`ラベルがなければ、`break`は内側の`for`からしか抜け出せません。
...
`into` メソッドは、ある型から別の型への変換が常に可能である場合のみ使用されます。もし型の値が変換できない場合、`TryInto` という別の型クラスと `tryInto` メソッドが使用されます。`tryInto` メソッドは `Optional` 型を返し、成功時は `Some`、失敗時は `None` になります。
