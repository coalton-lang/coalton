...
If you want to catch any error, you can use a wildcard pattern:
...
2. No way to `catch` a Lisp condition and bind it to a variable in a
   ...
   a Lisp exception.  Furthermore, you may only rethrow an excption by
...
4. No typeclass is associated with exception-signalling forms. We are
   ...
   might hop the calls stack. In the end, a typeclass approach may win
