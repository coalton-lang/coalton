...
(defun builder-default-candidates (env ambig)
  (let* ((var (ambiguity-var ambig))
         (from-collection (cached-builder-symbol
                           :from-collection
                           (lambda () (first (util:find-symbol? "FROMCOLLECTION" "COALTON/CLASSES")))))
         (from-association (cached-builder-symbol
                            :from-association
                            (lambda () (first (util:find-symbol? "FROMASSOCIATION" "COALTON/CLASSES")))))
    ...
    (loop :for pred :in (ambiguity-preds ambig)
          ...
          :when (and from-collection
                     seq-type
                     (eq (ty-predicate-class pred) from-collection)
                     (>= (length types) 2)
                     (ty= var (first types)))
            :do (multiple-value-bind (default-type)
                    (apply-type-argument seq-type (second types))
                  (return (list default-type)))
          :when (and from-association
                     seq-type
                     tuple-type
                     (eq (ty-predicate-class pred) from-association)
                     (>= (length types) 3)
                     (ty= var (first types)))
...
(defun expand-defaulted-builder-preds (env preds)
  (declare (type environment env)
           (type ty-predicate-list preds)
           (values ty-predicate-list &optional))
  (let ((from-collection (cached-builder-symbol
                          :from-collection
                          (lambda () (first (util:find-symbol? "FROMCOLLECTION" "COALTON/CLASSES")))))
        (from-association (cached-builder-symbol
                           :from-association
    ...
    (loop :for pred :in preds
          ...
          :if (or (eq class from-collection)
                  (eq class from-association))
