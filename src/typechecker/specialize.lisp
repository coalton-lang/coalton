  ...
  (let* ((from-name (parser:node-variable-name (parser:toplevel-specialize-from specialize)))
    ...
    (unless from-ty
      (tc-error "Invalid specialization"
                (make-note (parser:toplevel-specialize-from specialize)
                           "unknown function or variable")))
    (unless to-ty
      (tc-error "Invalid specialization"
                (make-note (parser:toplevel-specialize-to specialize)
                           "unknown function or variable")))

    (unless (eq :value (tc:name-entry-type from-name-entry))
      (tc-error "Invalid specialization"
                (make-note (parser:toplevel-specialize-from specialize)
                           (format nil "must be a function or variable, not a ~A"
                                   (tc:name-entry-type from-name-entry)))))
    ...
    (let ((from-qual-ty (tc:fresh-inst from-ty))
      ...
      (handler-case
        ...
        (tc:coalton-internal-type-error ()
          (tc-error "Invalid specialization"
                    (make-note specialize "cannot specialize to declared type"))))
