...
(defun keyword-stage-entry-table (entries)
  (declare (type keyword-ty-entry-list entries)
...
(defgeneric mgu (type1 type2)
  ...
  (:method ((type1 keyword-stage-ty) (type2 keyword-stage-ty))
    (let ((entries1 (keyword-stage-ty-entries type1))
          (entries2 (keyword-stage-ty-entries type2)))
      (unless (= (length entries1) (length entries2))
        (error 'unification-error :type1 type1 :type2 type2))
      (let ((table2 (keyword-stage-entry-table entries2))
            (subs nil))
        (dolist (entry1 entries1)
          (let ((entry2 (gethash (keyword-ty-entry-keyword entry1) table2)))
            (unless entry2
              (error 'unification-error :type1 type1 :type2 type2))
            (setf subs
                  (compose-substitution-lists
                   ...
                   subs))))
        (compose-substitution-lists
         (mgu (apply-substitution subs (keyword-stage-ty-to type1))
              (apply-substitution subs (keyword-stage-ty-to type2)))
         subs))))
          ...
          (let ((entry2 (gethash (keyword-ty-entry-keyword entry1) table2)))
            ...
            (setf subs
                  (compose-substitution-lists
                   ...
                   subs))))
        (compose-substitution-lists
         (match (apply-substitution subs (keyword-stage-ty-to type1))
                (apply-substitution subs (keyword-stage-ty-to type2)))
         subs))))
  (:method ((type1 tyvar) (type2 ty))
