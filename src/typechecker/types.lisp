...
(defgeneric instantiate (types type)
  (:method (types (type tapp))
    (make-tapp :from (instantiate types (tapp-from type))
...
(defmethod apply-ksubstitution (subs (type tapp))
  (make-tapp :from (apply-ksubstitution subs (tapp-from type))
...
(defvar *single-float-type*
  (make-tcon :tycon (make-tycon :name 'coalton:Single-Float :kind kstar)))
(defvar *double-float-type*
  (make-tcon :tycon (make-tycon :name 'coalton:Double-Float :kind kstar)))
(defvar *string-type* (make-tcon :tycon (make-tycon :name 'coalton:String       :kind kstar)))
(defvar *fraction-type*
  (make-tcon :tycon (make-tycon :name 'coalton:Fraction :kind kstar)))

(defvar *arrow-type* (make-tcon :tycon (make-tycon :name 'coalton:-> :kind (kfun kstar (kfun kstar kstar)))))

(defvar *list-type* (make-tcon :tycon (make-tycon :name 'coalton:List :kind (kfun kstar kstar))))
...
(defun make-function-type (from to)
  ...
  (make-tapp :from (make-tapp :from *arrow-type* :to from)
             :to to))
