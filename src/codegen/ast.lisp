(defpackage #:coalton-impl/codegen/ast
  ...
  (:export
   ...
   #:node-locally-noinline-applications-p ; READER
   #:node-locally-noinline-methods-p      ; READER
...
(defstruct (node-locally (:include node))
  ...
  (noinline-applications-p (util:required 'noinline-applications-p) :type boolean :read-only t)
  (noinline-methods-p      (util:required 'noinline-methods-p)      :type boolean :read-only t)
  (subexpr                 (util:required 'subexpr)                 :type node    :read-only t))
