...
(defgeneric traverse (node funs bound-variables)
  ...
  (:method ((node node-abstraction) funs bound-variables)
    ...
    (let ((node
            (make-node-abstraction
             ...
             :subexpr (traverse (node-abstraction-subexpr node) funs (append (node-abstraction-vars node) bound-variables)))))
  ...
  (:method ((node node-bare-abstraction) funs bound-variables)
    ...
    (let ((node
            (make-node-bare-abstraction
             ...
             :subexpr (traverse (node-bare-abstraction-subexpr node) funs (append (node-bare-abstraction-vars node) bound-variables)))))
  ...
  (:method ((node match-branch) funs bound-variables)
    ...
    (make-match-branch
     ...
     :body (traverse (match-branch-body node) funs (append (mapcar #'car (match-branch-bindings node)) bound-variables))))
