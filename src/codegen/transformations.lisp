...
(defun make-default-traversals ()
  ...
  (load-time-value
   (list
    (cons '(:traverse . node-application)
    ...
    (cons '(:traverse . node-direct-application)
          (lambda (node traverse &rest args)
            (declare (type function traverse)
    ...
    (cons '(:traverse . node-abstraction)
          (lambda (node traverse &rest args)
            (declare (type function traverse)
    ...
    (cons '(:traverse . node-dynamic-extent)
          (lambda (node traverse &rest args)
            (declare (type function traverse)
...
(defmethod tc:apply-substitution (subs (node node))
  ...
  (traverse
   ...
   (list
    ...
    (cons '(:after . node-direct-application)
          (lambda (node)
