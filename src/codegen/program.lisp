...
(defun compile-translation-unit (out translation-unit monomorphize-table env)
    ...
    (multiple-value-bind (definitions env)
      ...
      (let ((sccs (node-binding-sccs definitions)))
        ...
        (when (eq sb-ext:*block-compile-default* :specified)
          (emit out `(declaim (sb-ext:start-block ,@definition-names))))
        ...
        (loop :for scc :in sccs
              :for bindings
                := (remove-if-not
                    ...
                    definitions)
              :do (compile-scc out bindings env))

        #+sbcl
        (when (eq sb-ext:*block-compile-default* :specified)
          (emit out `(declaim (sb-ext:end-block))))
...
(defun compile-scc (out bindings env)
  ...
  (loop :for (name . node) :in bindings
        :when (node-abstraction-p node)
          :do (emit out (compile-function name node env))
              (emit out
  ...
  (loop :for (name . node) :in bindings
        :unless (node-abstraction-p node)
          :do (emit out `(setf ,name ,(codegen-expression node nil env))))
  ...
  (loop :for (name . node) :in bindings
        ...
        :when docstring
          :do (emit out `(setf (documentation ',name 'variable) ,docstring))
        :when (and entry (node-abstraction-p node))
          :do (emit out `(setf (documentation ',name 'function) ,docstring))))
