...
(defun compile-type-definition (def env)
  (let ((package (symbol-package (type-definition-name def))))
    ...
    (append
     ...
     (loop :for constructor :in (type-definition-constructors def)
           :for name := (constructor-entry-name constructor)
           :for codegen-docstring := (codegen-docstring name nil env)
           :collect
           `(setf (documentation ',name 'variable) ,codegen-docstring)

           :if (not (zerop (constructor-entry-arity constructor)))
           :collect
           `(setf (documentation ',name 'function) ,codegen-docstring)))))
