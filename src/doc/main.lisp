...
(defun write-documentation (filename packages &key (backend :markdown))
  "Write the documentation for a set of PACKAGES to FILENAME, using backend named by keyword BACKEND."
  (let ((*local* (namestring (asdf:system-source-directory "coalton")))
        (*remote* (format nil "http://github.com/coalton-lang/coalton/tree/~A" (head-revision))))
...
Possible values for BACKEND are:
  ...
  (write-documentation filename (find-packages) :backend backend))
