...
(define-test ordmap-basic-test ()
  ...
  (let ((src (list->ordmap (make-list (Tuple 1 "One")
                                      ...
                                      (Tuple 3 "Three")
                                      (Tuple 4 "Four")))))
    (let (values m v) = (ordmap:update src 4 (fn (prev) (values (Some "Quatre") prev))))
    (is (== (ordmap->list m)
            (the (List (Tuple Integer String))
                 (make-list (Tuple 1 "One")
                            ...
                            (Tuple 4 "Quatre")))))
    (is (== v (Some "Four")))
    (let (values m v) = (ordmap:update src 5 (fn (prev) (values (Some "Cinq") prev))))
    (is (== (ordmap->list m)
            (the (List (Tuple Integer String))
                 (make-list (Tuple 1 "One")
                            (Tuple 2 "Two")
    ...
    (is (== v None))
    (let (values m v) =
      (ordmap:update src 3 (fn (v)
                             (match v
                               ((None) (values (Some "Fill")
                                               True))
                               ((Some _) (values v False))))))
    (is (== (ordmap->list m)
            (the (List (Tuple Integer String))
                 (make-list (Tuple 1 "One")
                            (Tuple 2 "Two")
